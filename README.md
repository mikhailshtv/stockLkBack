...
## Email-уведомления (Kafka)

При создании, изменении или смене статуса заказа сервис публикует событие в топик Kafka `order-events`. Событие содержит тип операции, данные заказа и email пользователя.

```
stockLkBack → Kafka (order-events) → notification-service → SMTP → пользователь
```

Обработкой событий и отправкой писем занимается отдельный микросервис: [notification-service](https://github.com/mikhailshtv/notification-service).
